####### Still need to do some imports here
...
X[0].max
(X[0]/255).max()
# plt.imshow(x[0]) # showing pictures
...
class CifarHelper():
    ...
    def set_up_images(self):
        ...
        self.training_labels = one_hot_encode(np.hstack([d[b'data'] for d in self.all_train_batches]), 10)

        print('Setting up Test Images and Labels')
        self.test_images = np.vstack([d[b'data'] for d in self.test_batch])
        test_len = len(self.test_images)

        self.test_images = self.test_images.reshape(test_len, 3, 32, 32).transpose(0, 2, 3, 2)/255
